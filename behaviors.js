  ...
  if (tower.generateTimer >= (tower.generateInterval || 180)) {
    ...
    Object.entries(tower.resourceGenerator).forEach(([resourceId, amount]) => {
      ...
      if (resource) {
        ...
        // IMPROVED SEEK_AND_SHOOT WITH PREDICTIVE TARGETING
GameAPI.registerBehavior('SEEK_AND_SHOOT', function(tower) {
  tower.cooldownTimer = (tower.cooldownTimer || 0) - 1;
  
  let nearest;
  if (typeof TargetingManager !== 'undefined' && tower.targetFilter) {
    nearest = TargetingManager.getFilteredTarget(tower);
  } else {
    nearest = GameAPI.getNearestEnemy(tower.x, tower.y, tower.range);
  }
  ...
  if (nearest) {
    ...
    if (nearest.pathIndex < Game.path.length - 1) {
      const nextPoint = Game.path[nearest.pathIndex + 1];
      const dx = nextPoint.x - nearest.x;
      const dy = nextPoint.y - nearest.y;
      const dist = Math.hypot(dx, dy);
      
      if (dist > 0) {
        const prediction = 20; // Predict 20 frames ahead
        const moveDistance = nearest.speed * (nearest.speedMultiplier || 1) * prediction;
        targetX = nearest.x + (dx / dist) * moveDistance;
        targetY = nearest.y + (dy / dist) * moveDistance;
      }
    }
...
GameAPI.registerBehavior('RAPID_FIRE', function(tower) {
  ...
  if (nearest) {
    ...
    if (tower.cooldownTimer <= 0) {
      tower.cooldownTimer = tower.cooldown || 8;
      fireProjectile(tower, nearest);
...
GameAPI.registerBehavior('SNIPER_SHOT', function(tower) {
  ...
  if (nearest && !tower.charging) {
    ...
    if (nearest.pathIndex < Game.path.length - 1) {
      ...
      if (dist > 0) {
        const prediction = 40; // Long range needs more prediction
        const moveDistance = nearest.speed * (nearest.speedMultiplier || 1) * prediction;
        targetX = nearest.x + (dx / dist) * moveDistance;
        targetY = nearest.y + (dy / dist) * moveDistance;
      }
...
GameAPI.registerBehavior('FIRE_PROJECTILE', function(proj) {
  ...
  const hit = CollisionManager.checkProjectileCollision(proj, 10);
  
  if (hit) {
    const actualDamage = proj.damage * (proj.source.damageMultiplier || 1);
    GameAPI.damageEnemy(hit, actualDamage, proj.source);
    
    HookManager.trigger('projectileHit', proj, hit);
    
    if (proj.onHit) proj.onHit(hit, proj);
